...
def createWf():
    ...
    for i in range(N):
        ...
        graph.connect(incs[i], 'output', red, Redispatch.INPUT.format(i))
    
    for i in range(NN):
        graph.connect(red, Redispatch.OUTPUT.format(i), avg, MultiAverage.INPUT.format(i))
...
rid = 'MULTI_TO_MULTI_'+getUniqueId()

# Configuration setup of the provenance execution of the run
prov_config = {
    "provone:User": "rui",
    "s-prov:description": "multi to multi streaming",
    "s-prov:workflowName": "multi-to-multi",
